...
/** @type {typeof import('@discordjs/voice') | null} */
let voiceLib = null;
...
async function loadVoice() {
  ...
  try {
    ...
    logError({ event: 'VOICE_LIB_LOAD_FAIL', message: err.message });
    throw new Error(
      `Biblioteca de voz indisponível neste servidor (${err.message}). Status Streaming continua ok.`
    );
  }
}
...
export async function joinFixedVoiceChannel(client, channelId = defaultVoiceChannelId()) {
  ...
  try {
    const { getVoiceConnection, joinVoiceChannel, entersState, VoiceConnectionStatus } =
      await loadVoice();
...
export async function handleDesconectarCommand(interaction) {
  ...
  let conn = currentConnection?.joinConfig.guildId === interaction.guildId ? currentConnection : null;
  try {
    const { getVoiceConnection } = await loadVoice();
    conn = getVoiceConnection(interaction.guildId) || conn;
  } catch {
    /* voz lib indisponível — só limpa estado local */
  }
