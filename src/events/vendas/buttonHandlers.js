...
function createButtonHandlers({ client, dbMySQL, CONFIG, store, paymentService, avaliacaoService, discordLog, walletService }) {
    ...
    async function handleButtonInteraction(interaction) {
        ...
        if (interaction.customId === 'depositar_saldo') {
            const modal = new ModalBuilder()
                .setCustomId('modal_depositar_saldo')
                .setTitle('Depositar Saldo');

            const input = new TextInputBuilder()
                .setCustomId('deposito_valor')
                .setLabel('Valor do depósito (R$)')
        ...
        const isHookComprar = interaction.customId.startsWith('hooktrick_comprar_');
        if (interaction.customId.startsWith('ver_opcoes_') || isHookComprar) {
            const produtoId = isHookComprar
