...
function createButtonHandlers({ client, dbMySQL, CONFIG, store, paymentService, avaliacaoService, discordLog, walletService }) {
    ...
    async function handleButtonInteraction(interaction) {
        ...
        if (interaction.customId.startsWith('hooktrick_comprar_')) {
            const produtoId = interaction.customId.split('_').pop();
            ...
            const existingId = threadsPedidos.get(userId);
            if (existingId) {
                try {
                    const existing = await client.channels.fetch(existingId);
                    if (existing && !existing.archived) {
                        return interaction.editReply(buildPayload({
                            container: buildContainer({
                                title: '✅ Você já tem um tópico aberto',
                                description: `📋 **Ir para o carrinho** <#${existing.id}>`,
            ...
            const thread = await parent.threads.create({
                name: `🛒 ${interaction.user.username} - Hook Trick`,
                type: ChannelType.PrivateThread,
                invitable: false
            });

            await thread.members.add(userId);
            await helpers.addTopicMembers(thread, CONFIG.TOPIC_MEMBERS, [userId]);
            threadsPedidos.set(userId, thread.id);

            const bannerPath = path.join(__dirname, '../../../assets/hooktrick-banner.png');
            const files = [];
            if (fs.existsSync(bannerPath)) {
                files.push(new AttachmentBuilder(bannerPath, { name: 'hooktrick-banner.png' }));
            }
