...
function createExpressApp({ client, dbMySQL, CONFIG, store, registrarLog, paymentService }) {
    const app = express();
    ...
    app.post('/', async (req, res) => {
        const { usuario, senha, hwid } = req.body;
        ...
        try {
            ...
            if (rows.length > 0) {
                ...
                if (!userDb.hwid_vinculado || userDb.hwid_vinculado === '' || userDb.hwid_vinculado === null) {
                    await dbMySQL.query(
                        'UPDATE usuarios SET hwid_vinculado = ? WHERE usuario = ?',
                        [hwid, usuario]
                    );
                    console.log(`[AUTH] 🖥️ HWID Vinculado agora para ${usuario}: ${hwid}`);
                    return res.status(200).json({ success: true, message: 'HWID vinculado e acesso liberado!' });
                }

                if (userDb.hwid_vinculado !== hwid) {
                    console.log(`[AUTH] ⚠️ Troca de PC negada para: ${usuario}`);
                    return res.status(403).json({
                        success: false,
                        message: 'Este usuário já está vinculado a outro computador.'
                    });
                }

                console.log(`[AUTH] ✅ Login Sucesso: ${usuario}`);
