...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((sub) =>
            sub
                .setName('configure')
                .setDescription('Envia a loja. Comprar abre um tópico privado')
    ...
    async execute(interaction, client, dbMySQL) {
        try {
            ...
            const botao = new ActionRowBuilder().addComponents(
                new ButtonBuilder()
                    .setCustomId(`hooktrick_comprar_${prod.id}`)
                    .setLabel('Comprar')
                    .setEmoji('🛒')
            ...
            if (fs.existsSync(BANNER_PATH)) {
                files.push(new AttachmentBuilder(BANNER_PATH, { name: 'hooktrick-banner.png' }));
            }
            ...
            await channel.send(buildPayload({
                container: buildContainer({
                    ...
                    color: 0xffffff,
                    image: files.length ? 'attachment://hooktrick-banner.png' : null,
